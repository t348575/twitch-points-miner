<script lang="ts">
  ...
  const client = createClient<paths>({ baseUrl: window.location.href });
  ...
  const get_timeline = async (
    ...
    const { data, error } = await client.POST("/api/timeline", {
      body: {
  ...
  > => {
    const { data, error } = await client.GET("/api/live");
    if (error) {
      throw error;
    }
    return data;
  ...
  const get_last_prediction = async (
    ...
    const { data, error } = await client.GET("/api/live_prediction", {
      params: {
        query: {
          prediction_id,
          channel_id,
  ...
  async function place_bet() {
    let streamer = selected_streamer_for_prediction?.toString();
    ...
    await client.POST("/api/bet/{streamer}", {
      params: {
        path: {
          streamer: streamer.toString(),
        },
      },
      body: {
        event_id,
...
<main class="container min-w-full min-h-full pt-4 flex flex-col font-sans">
  <ModeWatcher />
  <h1 class="text-4xl text-center mb-8">Twitch points miner</h1>
  <div class="flex flex-col">
    <div class="flex flex-row">
      ...
      <div class="flex-1 mx-10">
        ...
        <VisXYContainer data={timeline} class="mt-10">
          <VisTooltip />
          <VisLine {x} {y} curveType="step" />
          <VisAxis
            type="x"
            label="Time"
      ...
      {#if prediction && !prediction_made}
        <Card.Footer class="flex flex-col">
          ...
          <div class="flex flex-row">
            ...
            >
            <p>Note: If prediction points is 0, the app runs the prediction logic without the filters </p>
          </div>
        </Card.Footer>
